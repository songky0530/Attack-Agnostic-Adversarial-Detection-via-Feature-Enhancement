...
save_path = f'KDE/{dataset}/Hessian_{hessian_dim}-LSCF_{LSCF_dim}-norm_{norm_feat}_cam/{kernel}_{bandwidth}_{adv_type}.txt'
os.makedirs(os.path.dirname(save_path), exist_ok=True)
if os.path.exists(save_path):
    exit()

adv_types = ['CW','DeepFool','APGD']
...
def load_data(
    ...
    if hessian_dim > 0:
        ...
        with open(file_path, 'rb') as IN:
            res = pickle.load(IN)
        # 兼容没有 num_processed 的情况
        valid_samples = res.get('num_processed', len(res.get('feature', [])))
        hessian_feature = format_data(res['feature'][:valid_samples])[:, : hessian_dim]
        hessian_feature = np.log(np.abs(hessian_feature) + 1e-10)

    LSCF_feature = None
    if LSCF_dim > 0:
        file_path = f'precomputed_LSCF_feature_cam/{dataset}/{net}_{adv_type}_{split}.pkl'
        with open(file_path, 'rb') as IN:
            res = pickle.load(IN)
